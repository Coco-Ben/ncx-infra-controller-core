...
The `InstanceStatus` field takes one of the following values: `Pending`, `Provisioning`, `Configuring`, `Ready`, `Updating`, `Rebooting`, `Terminating`, `Error`.
...
NICo emits MQTT events for machine state changes. Because MQTT messages can be lost in transit, and because machine health (not only state) is a required signal for determining readiness, polling the REST API is the recommended approach for tracking instance status.

**How does interface, InfiniBand, or NVLink status relate to instance readiness?**

All interfaces requested at instance creation must be fully configured before the instance transitions to `Ready`. Internally, the gRPC API exposes `configs_synced` fields for each subsystem (networking, InfiniBand, NVLink), while any subsystem remains unsynced:
...
The `MachineStatus` field takes one of the following values: `Initializing`, `Ready`, `Reset`, `Maintenance`, `InUse`, `Error`, `Decommissioned`, `Unknown`.
...
**What validation is performed after a compute instance reports `Ready`?**
...
**How can NSG and SSH key sync status be verified after instance creation?**
...
For NSGs, the gRPC API provides a method to check whether an NSG has been rolled out to all affected instances and DPUs. This capability is not currently exposed on the REST API.
...
**What are the standard remediation paths for a failed or stuck compute instance?**
